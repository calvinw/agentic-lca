...
| 11 | `impact-characterization` | ✅ Built | How each emission is multiplied by a characterization factor to produce an impact score — the maths behind the conversion |
| 12 | `damage-characterization` | ✅ Built | How midpoint impact scores are translated into damage to human health, ecosystems, and natural resources |
...
| 16 | `allocation` | ⬜ Recommended | What to do when a process produces more than one useful output — how to divide the environmental burden fairly between co-products |
| 17 | `normalization-and-weighting` | ⬜ Recommended | How to convert impact scores to a common scale so you can add them up into a single score — and why that's controversial |
| 18 | `sensitivity-analysis` | ⬜ Recommended | How to test whether results change when you swap an assumption — key for credibility and for understanding what really drives the score |
| 19 | `uncertainty` | ⬜ Recommended | Why LCA numbers are ranges, not precise values; Monte Carlo basics in plain language |
